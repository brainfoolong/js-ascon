...
(async function () {
  ...
  for (const variant in hash_variants) {
    ...
    for (let index = 0; index < messageLength + 1; index++) {
  ...
  for (const variant in cxof_variants) {
    ...
    let count = 1
    await globalThis.writeFile(__dirname + '/genkat_results/' + row.filename, fileData)
    for (let msgLen = 0; msgLen < messageLength + 1; msgLen++) {
      for (let customLen = 0; customLen < customizationLength + 1; customLen++) {
  ...
  for (const variant in auth_variants) {
    const messageLength = variant === 'Ascon-PrfShort' ? 16 : 1024
    const klen = 16
    const tlen = 16
  ...
  for (let variant in aead_variants) {
    ...
    let count = 1
    await globalThis.writeFile(__dirname + '/genkat_results/' + row.filename, fileData)
    for (let mlen = 0; mlen < messageLength + 1; mlen++) {
      for (let adlen = 0; adlen < assocDataLength + 1; adlen++) {
        let fileMsg = 'Count = ' + count + '\n'
        const fileDataStart = fileData.length
